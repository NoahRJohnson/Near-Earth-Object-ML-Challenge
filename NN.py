...
Feature Vectors are Right Ascension and Declination (these specify celestial coordinates)
 ...
 To Do: Include Date(?) and Magnitude (must normalize between different bands) features
...
with open(removed_fn, 'r') as names_f:
        ...
        try:
            with open(NEO_name, 'r') as observations_f:
                for observation in observations_f.readlines():
                    ...
                    removed_l.append(RA_h)
                    removed_l.append(RA_m)
                    removed_l.append(RA_ss)
                    removed_l.append(dec_d)
                    removed_l.append(dec_m)
                    removed_l.append(dec_ss)
...
with open(not_removed_fn, 'r') as names_f:
    for NEO_name in names_f.readlines():
        NEO_name = NEO_name + ".txt"
        try:
            with open(NEO_name, 'r') as observations_f:
